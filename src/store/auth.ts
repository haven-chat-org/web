...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  async login(username, password, totpCode) {
    ...
    // Restore E2EE sessions from local IndexedDB (survives logout)
    if (!identityChanged) {
      try {
        await loadCryptoState(res.user.id, identity.privateKey);
      } catch {
        // Local store not available — sessions will be re-established
      }
    }
  },
