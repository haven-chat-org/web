...
interface UiState {
  ...
  /** Export modal state (transient, not persisted) */
  exportModalOpen: boolean;
  exportModalContext: { type: "channel" | "dm" | "server"; id: string } | null;

  /** Download Attachments modal state (transient, not persisted) */
  ...
  openExportModal(context: { type: "channel" | "dm" | "server"; id: string }): void;
...
export const useUiStore = create<UiState>()(
  persist(
    (set, get) => ({
      ...
      openExportModal(context) { set({ exportModalOpen: true, exportModalContext: context }); },
