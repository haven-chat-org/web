...
function formatTtlLabel(ttl: number): string {
  if (ttl <= 30) return "30s";
  if (ttl <= 300) return "5m";
  if (ttl <= 3600) return "1h";
  if (ttl <= 28800) return "8h";
...
export default function Chat() {
  ...
  const attachmentsModalOpen = useUiStore((s) => s.attachmentsModalOpen);
  const openAttachmentsModal = useUiStore((s) => s.openAttachmentsModal);
            ...
            {showFriends && selectedServerId === null ? (
              ...
              <>
                ...
                {currentChannel?.message_ttl && (
                  <span className="chat-header-ttl" title={t("disappearingMessages.tooltip", { duration: formatTtlLabel(currentChannel.message_ttl) })}>
              ...
              <>
                ...
                <button
                  ...
                  onClick={() => openAttachmentsModal({
                    type: isDmOrGroupChannel ? "dm" : "channel",
                    id: currentChannelId!,
                  })}
                  title={t("attachments.headerButton")}
                  aria-label={t("attachments.headerButton")}
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
