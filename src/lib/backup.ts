...
/**
 * Restore sessions and sender keys from a BackupPayload into memory.
 * Used by both server backup restore and local IndexedDB restore.
 * Does NOT touch identity/signed prekey or auth store — caller handles that.
 */
export function restoreCryptoFromPayload(payload: BackupPayload): void {
...
export async function downloadAndRestoreBackup(securityPhrase: string): Promise<void> {
  ...
  // Restore sessions and sender keys
...
// ─── Auto-Backup & Local Persist ────────────────────────

let autoBackupTimer: ReturnType<typeof setTimeout> | null = null;
let localPersistTimer: ReturnType<typeof setTimeout> | null = null;
const AUTO_BACKUP_DELAY_MS = 5000;
const LOCAL_PERSIST_DELAY_MS = 5000;

/**
 * Schedule a debounced auto-backup and local IndexedDB persist.
 * Server backup only runs if a security phrase is cached.
 * Local persist always runs (uses identity-key-derived encryption).
 * Call after key material changes (new session, new sender key, etc.).
 */
export function scheduleAutoBackup(): void {
  // Server backup (gated on security phrase)
  if (cachedPhrase) {
  ...
  // Local IndexedDB persist (always, no phrase needed)
  if (localPersistTimer) clearTimeout(localPersistTimer);
  localPersistTimer = setTimeout(async () => {
    localPersistTimer = null;
    const { user, identityKeyPair } = useAuthStore.getState();
    if (!user || !identityKeyPair) return;
    try {
      const { persistCryptoState } = await import("./crypto-store.js");
      await persistCryptoState(user.id, identityKeyPair.privateKey);
    } catch (e) {
      console.warn("Local crypto persist failed:", e);
    }
