/**
 * High-level E2EE helpers that bridge @haven-chat-org/core crypto with the Haven backend.
 *
...
export async function ensureSenderKeyDistributed(channelId: string): Promise<void> {
  ...
  if (!mySenderKeys.has(channelId)) {
    ...
    // Store a cloned copy in receivedSenderKeys so we can decrypt our own
    // group messages after session restore (decryptIncoming only checks receivedSenderKeys)
    const sk = mySenderKeys.get(channelId)!;
    const selfCacheKey = senderKeyCacheId(channelId, sk.distributionId);
    const userId = useAuthStore.getState().user?.id;
    if (userId) {
      receivedSenderKeys.set(selfCacheKey, {
        fromUserId: userId,
        key: {
